...
-----------------------------------------------------------  
### 나의 해석 :  
세 점이 일직선에 위치하는 경우는 없다고 했으니, 정점들이 들어올때마다 사이클이 생기는지 확인해주면 된다.  
사이클을 확인하는 방법은 점들이 이어진 집합을 분리 집합으로 표현하였을 때, 점들을 잇기 전에 두 점이 같은 집합에 있다면 두 점을 이었을 때,  
사이클이 발생한다는 것이기 때문에 해당 지점에서 cycle이 발생하는 차례를 저장하면 된다.  
